  ...
  ( ans_ty,
    ...
    lift
      [
        ...
        (ans_id, Bitcast (mal_ty, Id str_id, ans_ty));
      ] )
...
let rec cmp_exp (tc : TypeCtxt.t) (c : Ctxt.t) (exp : Ast.exp node) :
  ...
  *)
  | Ast.Length e ->
      ...
      (* let len_ptr_int = gensym "len_ptr_int" in *)
      let op_id = gensym "len_var" in
      ( I64,
        Id op_id,
        arr_code
        >:: I (len_ptr, Gep (arr_ty, arr_op, [ Const 0L; Const 0L ]))
        (* >:: I (len_ptr_int, Bitcast (arr_ty, Id len_ptr, Ptr I64)) *)
        >:: I (op_id, Load (Ptr I64, Id len_ptr)) )
  | Ast.Index (e, i) ->
  ...
  *)
  | Ast.NewArr (elt_ty, e1, id, e2) ->
      ...
      and after =
        no_loc (Assn (id_node, no_loc (Bop (Add, id_node, no_loc @@ CInt 1L))))
      and body = [ no_loc @@ Assn (no_loc @@ Index (arr_node, id_node), e2) ] in
      let for_stmt = no_loc @@ For (init, Some guard, Some after, body) in
      let _, for_code = cmp_stmt tc up_c Void for_stmt in
      (arr_ty, arr_op, size_code >@ alloc_code >@ array_acc_code >@ for_code)
        ...
        List.concat
          (List.mapi
             (fun i field ->
               ...
               let cast_field_ptr = "cast" ^ field_ptr in
               let bitcast_c =
                 match t with
                 | Ptr (Struct [ I64; Array (j, t_a) ]) ->
                     if j <> 0 then
                       [
                         I
                           ( cast_field_ptr,
                             Bitcast
                               ( Ptr (Ptr (Struct [ I64; Array (0, t_a) ])),
                                 Id field_ptr,
                                 Ptr t ) );
                       ]
                       >@ [ I ("", Store (t, op, Id cast_field_ptr)) ]
                     else [ I ("", Store (t, op, Id field_ptr)) ]
                 | _ -> [ I ("", Store (t, op, Id field_ptr)) ]
               in
               c_e
               >:: I
                     ( field_ptr,
                       Gep (tstr, opstr, [ Const 0L; Const (Int64.of_int i) ])
                     )
               >@ bitcast_c)
             str_ty)
      in
  ...
  *)
  | Ast.Proj (e, i) ->
      ...
      let str_id =
        match str_ty with
        | Ptr (Namedt id) -> id
        | _ -> "Invalid struct for procj"
      in
      let ty_f, ind_f = TypeCtxt.lookup_field_name str_id i tc in
      let ll_ty_f = cmp_ty tc ty_f in
      let field_ptr = gensym "field_ptr" in
      ( ll_ty_f,
        Id field_ptr,
        str_code
        >@ [ I (field_ptr, Gep (str_ty, str_op, [ Const 0L; Const ind_f ])) ] )
  ...
  *)
  | Ast.Cast (typ, id, exp, notnull, null) ->
      ...
      ( new_ctxt,
        ...
        >:: L notnull_lbl
        >:: E (id, Alloca exp_ty)
        >@ notnull_code >:: T (Br merge_lbl) >:: L null_lbl >@ null_code
        >:: T (Br merge_lbl) )
...
let rec cmp_gexp c (tc : TypeCtxt.t) (e : Ast.exp node) :
  ...
  | CStruct (id, cs) ->
      let str_fields = TypeCtxt.lookup id tc in
      let elts, gs =
        List.fold_right
          (fun field (l, gs) ->
            let exp = List.assoc field.fieldName cs in
            let gd, glist = cmp_gexp c tc exp in
            (gd :: l, glist @ gs))
          str_fields ([], [])
      in
      let gid = gensym @@ id ^ "_g_struct" in
      let struct_i = GStruct elts in
      ((Ptr (Namedt id), GGid gid), (gid, (Namedt id, struct_i)) :: gs)
