...
let meaning_of_life : int =
  mult_by_3 14 (* call the partially applied function *)
...
*)
let centimes_of : int -> int -> int = fun (x : int) (y : int) -> x + (100 * y)
...
let compose_pair (p : ('b -> 'c) * ('a -> 'b)) : 'a -> 'c =
  match p with x, y -> fun (z : 'a) -> x (y z)
...
let is_mylist_empty (l : 'a mylist) : bool =
  match l with Nil -> true | Cons (h, tl) -> false
...
let rec list_to_mylist (l : 'a list) : 'a mylist =
  match l with [] -> Nil | h :: t -> Cons (h, list_to_mylist t)
...
let rec append (l1 : 'a list) (l2 : 'a list) : 'a list =
  match l1 with
  | h :: t -> h :: append t l2
  | [] -> ( match l2 with h :: t -> h :: append l1 t | [] -> [])
...
let rec insert (x : 'a) (l : 'a list) : 'a list =
  match l with
  | [] -> x :: []
  | n :: l1 -> if n > x then x :: l else if n = x then l else n :: insert x l1
...
let rec union (l1 : 'a list) (l2 : 'a list) : 'a list =
  match l1 with [] -> l2 | n :: l -> union l (insert n l2)
...
let op_as_string (ast : exp) : string =
  match ast with Add _ -> " + " | Mult _ -> " * " | _ -> raise Not_found

let rec string_of (e : exp) : string =
  match e with
  | Var s -> s
  | Add (e1, e2) | Mult (e1, e2) ->
      let op = op_as_string e in
      "(" ^ string_of e1 ^ op ^ string_of e2 ^ ")"
  | Neg e1 -> "-(" ^ string_of e1 ^ ")"
...
(*
  Problem 4-3like, you had norm 2 than you went for norm 1
...
let ctxt3 = append ctxt2 [ ("z", 0L) ]
let e4 = Mult (e3, Add (Var "z", e3))
let e5 = Add (Const 3L, Mult(Const 0L , Var "x"))

let rec optimize (e : exp) : exp =
  (* let () = print_endline (string_of e) in *)
  match e with
