...
%left AND
%left EQEQ NOTEQ
%left LTHAN LTEQ GTHAN GTEQ
%left SHIFTL SHIFTR SHIFTRAR
%left PLUS DASH
%left STAR
...
gexp:
  ...
  | NEW t=ty LRBRACKET LBRACE gs=separated_list(COMMA,gexp) RBRACE
...
exp:
  ...
  | e=exp LPAREN es=separated_list(COMMA, exp) RPAREN
                        { loc $startpos $endpos @@ Call (e,es) } /* exp instead of id?*/
  | LPAREN e=exp RPAREN { e } 
  | s= STRING           { loc $startpos $endpos @@ CStr s}
  | FALSE               { loc $startpos $endpos @@ CBool false}
  | TRUE                {loc $startpos $endpos @@ CBool true}
  | NEW t=ty LRBRACKET LBRACE  es=separated_list(COMMA,  exp) RBRACE 
...
vdecls:
  | vs=separated_list(COMMA, vdecl) {vs}      /*vdecls ??*/
